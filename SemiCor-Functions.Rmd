...
#' @param folds A vector representing the fold number of the corresponding y_hat and y value.
...
# Creating Examples with Online R Studio Datasets
## Using Neuro Datasets Package
### Installing data 
```{r}
# Setting the seed
set.seed(123)
...
# Loading in dataset of interest
...
data_clean = OASIS_cross_tbl_df %>% drop_na(Age) 
...
for(i in 1:folds){
  ...
  # Creating model data variables 
  x_var = as.matrix(train[, colnames(data_clean) %in% c("eTIV", "ASF")])
  y_var = as.matrix(train[, colnames(data_clean) %in% c("Age")])
  newx = as.matrix(test[, colnames(data_clean) %in% c("eTIV", "ASF")])
  newy = as.matrix(test[, colnames(data_clean) %in% c("Age")])
  ...
  # Storing values
  preds_eTIV <- c(preds_eTIV, test$preds)
  test_y_eTIV = c(test_y_eTIV, newy)
  fold <- c(fold, rep(i, length(test$preds)))
...
for(i in 1:folds){
  ...
  # Creating model data variables 
  x_var = as.matrix(train[, colnames(data_clean) %in% c("nWBV", "ASF")])
  y_var = as.matrix(train[, colnames(data_clean) %in% c("Age")])
  newx = as.matrix(test[, colnames(data_clean) %in% c("nWBV", "ASF")])
  newy = as.matrix(test[, colnames(data_clean) %in% c("Age")])
  ...
  # Storing values
  preds_nWBV <- c(preds_nWBV, test$preds)
  test_y_nWBV = c(test_y_nWBV, newy)
...
### Applying Semicor to Data Predictions
```{r}
# Making sure all are a matrix
preds_eTIV = as.matrix(preds_eTIV)
preds_nWBV = as.matrix(preds_nWBV)
fold = as.matrix(fold)


# Gathering the estimates with one sample split
results = sampleSplitOS(test_y_eTIV, preds_eTIV, fold, preds_nWBV)
```

### Graphing Outputs
```{r}
# Subsetting results to only estimates
toPlot = results$result %>% filter(metric == "yhat1" | metric == "yhat2")
...
### Analysis
From these results, we see there is no significant improvement in Ridge Regression model performance for models using eTIV over nWBV imaging data. 
